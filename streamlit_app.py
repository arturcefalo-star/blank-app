...
# CORREÇÃO DE SEGURANÇA: Tratamento do LocalStorage sem quebrar se faltar 'dados_completos'
try:
...
for k, v in contas_locais.items():
    if k not in todos_usuarios_server and "dados_completos" in v:
        ...
        if usuarios_db_dev:
            for i, (key_jogador, dados_user) in enumerate(list(usuarios_db_dev.items())):
                ...
                if col_dev_pts.button("Pontos", key=f"dev_pts_{key_jogador}_{i}"):
                    novos_pontos = max(0, usuarios_db_dev[key_jogador]["dados"].get("pontos", 0) + qtd_alteracao)
                ...
                if col_dev_clk.button("Poder/C", key=f"dev_clk_{key_jogador}_{i}"):
                    novo_poder = max(1, usuarios_db_dev[key_jogador]["dados"].get("poder_base", 1) + qtd_alteracao)
                ...
                if col_dev_pps.button("Pontos/s", key=f"dev_pps_{key_jogador}_{i}"):
                    novo_pps = max(0, usuarios_db_dev[key_jogador]["dados"].get("pontos_por_segundo", 0) + qtd_alteracao)
...
def renderizar_loja_totens():
    ...
    for idx, totem in enumerate(totens_da_rodada):
        with colunas_totens[idx]:
            ...
            if st.button("Comprar Totem", key=f"btn_buy_totem_{idx}", disabled=btn_desativado, use_container_width=True):
...
if st.session_state.mundo_atual == 2:
    ...
    # CORREÇÃO: Valor 500005 alterado para 500000
    melhorias_clique = [
        {"qtd": 1, "custo": 100},
        {"qtd": 5, "custo": 500},
        {"qtd": 10, "custo": 1000},
